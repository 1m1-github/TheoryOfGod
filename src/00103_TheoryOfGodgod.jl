...
function dxdy(g::god)
    ...
    dxort = dx .- d[end-2] / dnorm * d
    dxnorm = g.norm(dxort)
    dy = SA[zeros(T, N - 2)..., one(T), zero(T)]
    dyort = dy .- d[end-1] / dnorm * d
    dynorm = g.norm(dyort)
    dz = SA[zeros(T, N - 1)..., one(T)]
    dzort = dz .- d[end] / dnorm * d
    dznorm = g.norm(dzort)
    dx = dxort
    dy = dyort
